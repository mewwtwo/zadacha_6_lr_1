...
// Власний клас функціональних об'єктів
class Generator {
    vector<string> q;
    size_t n;
    // Бібліотечний функціональний об'єкт less<Variants> для впорядкування у множині
    set<Variants, less<Variants>> used;
    mt19937 rng{random_device{}()};
...
public:
    ...
    string operator()() {
        ...
        do {
            ...
            // Генеруємо n випадкових унікальних індексів
            while (ticket.indices.size() < n) {
                ticket.indices.insert(dist(rng));
            }
        } while (used.contains(ticket)); // перевірка наявності (використовує <=> / operator==)
